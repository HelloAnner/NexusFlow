...
首阶段先实现不引入额外依赖的轻量命令面板，后续再按需要替换为 `cmdk` / `react-hotkeys-hook`。该切片必须满足以下可验收行为：

- 顶部搜索入口是命令面板触发器，不直接提交搜索路由。
- `Cmd/Ctrl + K` 打开命令面板，`Esc` 关闭，`Cmd/Ctrl + N` 直接进入新建任务。
- 面板打开后焦点进入搜索输入框，关闭后回到原页面上下文。
- 未输入时展示 Navigate / Actions / Recent 三组，本地导航和核心动作不依赖搜索服务。
- 输入普通关键词时调用 `/api/search?q=`，并按任务、项目、人员、资料分组展示；搜索接口失败时保留 Navigate / Actions 组并显示错误提示。
- 输入 `/new task`、`/go inbox`、`/go schedule`、`/go conflicts` 等命令时直接展示可执行动作。
- 支持 `ArrowUp/ArrowDown`、`J/K` 移动选择，`Enter` 打开选中项。
- 打开对象或执行命令后，把该项写入本地 Recent，最多保留 8 条。
- 侧边栏展开宽度 220px，折叠宽度 52px；折叠态只显示图标，文字通过 `title` 保留可发现性。
- 侧边栏折叠状态写入 `localStorage`，键名为 `nexusflow.sidebar.collapsed`；移动端抽屉始终使用展开态。
